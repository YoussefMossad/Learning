...
app.post('/api/enrollmentsExams', (req, res) => {
    ...
    const sql = `
    INSERT INTO enrollments_exams (enrollments_id, exam_id) 
    VALUES (
    (
...
app.get('/api/notifications/:id', (req, res) => {
    const { id } = req.params;
    const sql = `
    SELECT n.id, n.message, n.is_read,DATE_FORMAT(created_at, '%m-%d %H:%i') AS created_at 
...
app.get('/api/unread-notifications/:id', (req, res) => {
    const { id } = req.params;
    const sql = `
    SELECT COUNT(is_read) as unread FROM notifications as n 
    INNER JOIN enrollments as e ON n.instructor_course_id = e.instructor_course_id
